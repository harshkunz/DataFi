...
import { LayoutDashboard, ArrowLeftRight, Lightbulb, ChevronLeft, ChevronRight, Receipt, Target, RotateCcw } from "lucide-react";
...
export function Sidebar() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("_sidebar_collapsed");
    if (stored) setCollapsed(stored === "true");
  }, []);

  const toggle = () => {
    setCollapsed((c) => {
      localStorage.setItem("_sidebar_collapsed", String(!c));
      return !c;
    });
  };
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex items-center h-16 px-4 border-b border-border", collapsed ? "justify-center" : "gap-2")}>
        ...
        {!collapsed && <span className="font-bold text-lg tracking-tight"> DataFi </span>}
      </div>
